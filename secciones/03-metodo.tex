...
Esta sección utiliza la siguiente terminología:
\begin{itemize}
    \item \emph{Extensión}: aplicación de navegador que extrae características del DOM.
    \item \emph{Motor de riesgo}: conjunto de servicios que estiman el nivel de riesgo
    (excluye persistencia).
    \item \emph{Sistema}: la arquitectura completa (extensión + motor de riesgo + persistencia).
\end{itemize}
...
Esta distinción separa el presente trabajo de la línea dominante en detección de fraude, cuya
dependencia de telemetría interna ya mencionada. El comprador
dispone de un conjunto de características restringido, por lo que la pregunta central que este
trabajo aborda es si dichos datos accedibles resultan suficientes para producir una advertencia
útil.
...
primera observación se declara como amenaza a la validez más adelante. La segunda es la
...
pago hasta la conformidad del comprador o garantizan su devolución ante incumplimiento. Dicha
declaración constituye un compromiso público y verificable en la propia ficha. Representa por
ello una característica legítima.

Su ausencia, en cambio, no es igualmente informativa: un vendedor independiente puede ofrecer
condiciones equivalentes sin anunciarlas, o anunciarlas donde la extensión no alcanza a leer.
Tratar el silencio como evidencia de inseguridad convertiría una limitación del observador en
una acusación sobre el observado. Esto es tanto más problemático cuanto que las contramedidas
disponibles se despliegan y financian del lado del comercio \cite{ref30}. Su ausencia visible
correlaciona con el tamaño del vendedor antes que con su honestidad.

La arquitectura se organiza en tres capas (cliente, motor de riesgo y persistencia) y se
detalla más adelante. La separación en tres capas responde a la necesidad
de desacoplar la observación del DOM (que debe ejecutarse en el cliente por razones de
privacidad y eficiencia) del análisis de riesgo (que requiere acceso a servicios externos) y
de la persistencia (que debe garantizar la trazabilidad sin exponer datos sensibles). Esta
separación permite escalar cada componente de forma independiente. Facilita la sustitución de
algoritmos de clasificación sin modificar la capa de observación.
...
vector de observaciones que envía al motor de riesgo. La extensión constituye el componente
...
El flujo de operación de la capa cliente se resume en cuatro pasos: (1) el usuario abre una
ficha de producto; (2) la extensión detecta la carga y extrae características del DOM; (3)
construye un vector de observaciones; y (4) envía el vector al motor de riesgo mediante
petición HTTP segura. La extensión no almacena datos sensibles del usuario.

Un cliente complementario consume el mismo servicio y cubre la compra que ocurre fuera del
navegador de escritorio. Opera sobre lo que el usuario le proporciona, sin acceso al documento
renderizado, de modo que resolverá menos características.
...
El motor de riesgo trabaja sobre ocho características, resumidas en la tabla de características.
...
estados la dejan sin resolver, y la tabla de características indica cuál adopta cada una
cuando la ficha no aporta evidencia. El agregador propaga esa distinción hasta el veredicto
...
La elección de ocho características se justifica por la necesidad de cubrir las tres dimensiones
con al menos dos indicadores por dimensión, lo que permite detectar inconsistencias y reducir
la influencia de valores atípicos. Un conjunto menor limitaría la capacidad de discriminación;
uno mayor aumentaría la complejidad de extracción sin beneficio proporcional en la precisión
del veredicto, dada la información disponible en el lado del cliente.
...
evalúa el historial y las políticas del vendedor; una API de estimación de riesgo calcula la
probabilidad de estafa mediante un bosque aleatorio; y una API de detección de reseñas falsas
...
El flujo de operación del motor de riesgo se resume en cuatro pasos: (1) la API centralizadora
recibe el vector de observaciones de la extensión; (2) delega las características relevantes
a los cuatro servicios; (3) cada servicio procesa de forma independiente y en paralelo; y (4)
la API centralizadora combina los resultados y almacena la consulta en la base de datos. Esta
separación permite escalar horizontalmente cada componente según la carga.

\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figuras/arquitectura-logica.jpeg}
\caption{Arquitectura lógica propuesta. La capa cliente (extensión de navegador) extrae
características del DOM renderizado y las envía a la API centralizadora del motor de riesgo.
El motor de riesgo se descompone en cuatro servicios: un analizador de veracidad de dominio,
un analizador de reputación, una API de estimación de riesgo y una API de detección de
reseñas falsas. La API centralizadora agrega los resultados y almacena las consultas en la
capa de persistencia para trazabilidad y reentrenamiento.}
\label{fig:arquitectura}
\end{figure*}

\begin{table}[t]
\caption{Componentes de la arquitectura y sus funciones principales}
...
\toprule
\textbf{Componente} & \textbf{Capa} & \textbf{Función} \\
\midrule
Ficha de Producto & Cliente & Fuente de datos (DOM renderizado) \\
Extensión de navegador & Cliente & Extrae 8 características del DOM \\
Aplicación web & Cliente & Autenticación y acceso a servicios \\
API centralizadora & Motor de riesgo & Recibe, delega y agrega resultados \\
Analizador de dominio & Motor de riesgo & Verifica veracidad del sitio web \\
Analizador de reputación & Motor de riesgo & Evalúa historial del vendedor \\
API de estimación de riesgo & Motor de riesgo & Calcula probabilidad de estafa \\
API de detección de reseñas & Motor de riesgo & Identifica contenido fraudulento \\
Base de Datos & Persistencia & Almacena para trazabilidad y reentrenamiento \\
\bottomrule
\end{tabular}
\end{table}
...
cadena de resolución incluidos los registros de nombre canónico (CNAME) y de la coincidencia
con patrones morfológicos asociados a campañas de suplantación.
...
lugar de contenido es que estos atributos son costosos de falsificar. Se ha mostrado que
...
El flujo de operación del analizador de dominio se resume en tres pasos: (1) recibe el nombre
de host de la extensión; (2) consulta servicios de registro y resolución de nombres para
obtener atributos del dominio; y (3) aplica reglas de clasificación para emitir una estimación
ordinal de veracidad (alta, media o baja). El módulo opera de forma asincrónica y no bloquea
la emisión del veredicto principal.
...
calificación que la acompaña. La brevedad del ajuste   una sola época   constituye una
limitación que la Sección~IV recoge. Este trabajo no evalúa el
clasificador de reseñas: la tabla de configuración documenta su configuración para hacer
reproducible el componente. Ni el corpus de ajuste ni sus métricas de prueba forman parte de
lo reportado aquí, y su evaluación queda comprendida en el trabajo futuro de la Sección~V.
...
La interfaz extiende el botón estándar del navegador. Al hacer clic, el usuario visualiza: (1)
un indicador de color con el veredicto (rojo para riesgo alto, amarillo para moderado, verde
para bajo, gris para evidencia insuficiente); (2) la lista de características verificadas con
su valor individual; (3) las características no resueltas y la razón de la falla; y (4) un
botón para acceder al registro de la consulta.
...
constituye el conjunto de datos con el que los modelos se reentrenarán. La Sección~IV fija los
términos de retención y anonimización.